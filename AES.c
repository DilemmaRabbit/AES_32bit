...
void AES_encrypt(unsigned char *state, char *key, int N){
...
void AES_decrypt(unsigned char *state, char *key, int N){
    unsigned int temp[4] = {0};
    int i,j;
    for(i=N+6;i>=0;i--){
        if(i==0){
            ...
            for(j=0;j<4;j++){
                temp[j] = td0[*(state+(j*4))] ^ td1[*(state+((13+j*4)%16))] ^ td2[*(state+((10+j*4)%16))] ^ td3[*(state+((7+j*4)%16))];
                printf("%x\n",temp[j]);
            }
