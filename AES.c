...
//generate inv_key table
void CreateInvKey(unsigned char *key, int N){
    int i;
    showkey(key,N);
    for(i=1;i<N+6;i++){
        InvMixColumn(key+16*i);
    }
    showkey(key,N);
}
...
void AES_encrypt(unsigned char *state, char *key, int N,int fast){
    ...
    for(i=0;i<N+7;i++){
        if(i==0){
            ...
            if(fast==1){
                for(j=0;j<4;j++){
                    temp[j] = te0[*(state+j*4)] ^ te1[*(state+(((1+j)*4)+1)%16)] ^ te2[*(state+(((2+j)*4)+2)%16)] ^ te3[*(state+(((3+j)*4)+3)%16)];
                }
                for(j=0;j<4;j++){
                    for(k =0;k<4;k++){
                        state[4*j+k] = temp[j] >> (24-8*k);
                    }
                }
            }
            //basic version
...
void AES_decrypt(unsigned char *state, char *key, int N,int fast){
    unsigned int temp[4] = {0};
    int i,j;
    for(i=N+6;i>=0;i--){
        if(i==0){
